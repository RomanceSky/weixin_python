...
class WeixinInterface:
    ...
    def POST(self):        
        ...
        if msgType == 'text':
            ...
            if content == 'help': 
                return self.render.reply_text(toUser, fromUser, int(time.time()), "随便看看？（对不起我功能有限QAQ）")
            else:
                return self.render.reply_text(toUser, fromUser, int(time.time()), "哎呀出错了 输入个help看看如何正确的调戏我？")
        if msgType == 'event':
